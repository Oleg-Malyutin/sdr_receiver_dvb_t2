...
//            break;
    ...
    _sample_rate_offset = (dph_2 + dph_1) / static_cast<float>(len_show);

    if(idx_symbol == n_p2){
        int len = 1024;
        if(len < c_data) len = c_data;
        memcpy(show_symbol, _ofdm_cell, sizeof(complex) * static_cast<unsigned long>(fft_size));
        memcpy(show_data, deinterleaved_cell, sizeof(complex) * static_cast<unsigned long>(c_data));
