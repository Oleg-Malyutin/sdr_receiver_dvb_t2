...
public:
    ...
    {
        in = static_cast<fftwf_complex*>(fftwf_malloc(sizeof(fftwf_complex) * static_cast<uint>(_len_in)));
        out_fft = static_cast<fftwf_complex*>(fftwf_malloc(sizeof(fftwf_complex) * static_cast<uint>(_len_in)));
        plan = fftwf_plan_dft_1d(_len_in, in, out_fft, FFTW_FORWARD, FFTW_ESTIMATE);
        out = static_cast<fftwf_complex*>(fftwf_malloc(sizeof(fftwf_complex) * static_cast<uint>(_len_in)));
