...
#if WIN32
#define M_PI_2f32 (3.14159f / 2.0f)
...
struct table_sin_cos
{
    void table_()
    {
        for (int i = -32767; i < 32768; i++){
            look_up_table_sin[i + 32767] = sin(i / k_table);
            look_up_table_cos[i + 32767] = cos(i / k_table);
        }
    }
...
#else
static float look_up_table_sin[131072];
static float look_up_table_cos[131072];
static const float k_table = 65535.0f / (2.0f * M_PIf32);
//TO DO: need initialize table_sin_cos_instance.table_()
struct table_sin_cos
{
    void table_()
    {
        for (int i = -65535; i < 65536; i++){
            look_up_table_sin[i + 65535] = sin(i / k_table);
            look_up_table_cos[i + 65535] = cos(i / k_table);
        }
    }
} static table_sin_cos_instance;
inline float sin_lut(const float &_x){return look_up_table_sin[int(_x * k_table + 65535) & 131071];}
inline float cos_lut(const float &_x){return look_up_table_cos[int(_x * k_table + 65535) & 131071];}
#endif
inline float atan2_approx(const float &_y, const float &_x)
{
    ...
    float abs_x = std::abs(_x);
    float abs_y = std::abs(_y);
